...
const server = new Server({ name: "memory-engram", version: "0.1.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "remember",
            description: "Guarda un hecho o decision en memoria persistente de largo plazo. Usa esto solo para informacion reutilizable en sesiones futuras.",
            inputSchema: {
                type: "object",
                properties: {
                    text: { type: "string" },
        ...
        switch (name) {
            ...
            case "recall": {
                ...
                const formatted = results
                    .map((m) => `#${m.id} (score ${m.score.toFixed(2)}, importancia ${m.importance}): ${m.text} [tags: ${m.tags.join(", ") || "sin tags"}]`)
                    .join("\n");
